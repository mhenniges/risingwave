...
//! Manager for the Iceberg V3 sink path. Owns per-sink commit workers that
...
//! per-sink compaction will live alongside the per-sink commit worker here.
...
/// way. Used at CREATE SINK time and during recovery to (re-)spawn the V3
/// commit worker.
